...
public class AudioController_Editor : EditorEx
{
    ...
    const string _nameForNewCategoryEntry = "!!! Enter Unique Category PlayerName Here !!!";
    ...
    public override void OnInspectorGUI()
    {
        ...
        if ( categoryFoldout = EditorGUILayout.Foldout( categoryFoldout, "Category Settings", foldoutStyle ) )
        {
            ...
            if ( curCat != null )
            {
                ...
                EditString( ref curCat.Name, "PlayerName", curCat.Name == _nameForNewCategoryEntry ? textAttentionStyle : null );
                ...
                /*if ( GUILayout.Button( "Add all items in this category to playlist" ) )
                {
                    ...
                    {
                        ArrayHelper.AddArrayElement( ref AC.musicPlaylist, curCat.AudioItems[i].PlayerName );
                    }
                ...
                if ( itemFoldout = EditorGUILayout.Foldout( itemFoldout, "Audio Item Settings", foldoutStyle ) )
                {
                    ...
                    GUILayout.Label( "use inspector lock or drag&drop here!" );
                    EditorGUILayout.EndHorizontal();
					
					UnityEngine.AudioClip[] dropClips = null;
					if (DropAudioClipsInArea(GUILayoutUtility.GetLastRect(), ref dropClips))
					{
                        if ( dropClips.Length > 0 )
                        {
                            int firstIndex = itemCount;
                            currentItemIndex = firstIndex;
                            foreach ( AudioClip audioClip in dropClips )
                            {
                                ArrayHelper.AddArrayElement( ref curCat.AudioItems );
                                AudioItem audioItem = curCat.AudioItems[ currentItemIndex ];
                                audioItem.Name = audioClip.name;
                    ...
                    if ( curItem != null )
                    {
                        ...
                        if ( EditString( ref curItem.Name, "PlayerName", isNewDummyName ? textAttentionStyle : null, "You must specify a unique name here (=audioID). This is the ID used in the script code to play this audio item."  ) )
                        {
                            if ( !isNewDummyName )
                            {
                        ...
                        {
                            AddToPlayList( curItem.PlayerName );
                        }*/
                        ...
                        if ( subitemFoldout = EditorGUILayout.Foldout( subitemFoldout, "Audio Sub-Item Settings", foldoutStyle ) )
                        {
							...
							UnityEngine.AudioClip[] dropClips2 = null;
							if (DropAudioClipsInArea(GUILayoutUtility.GetLastRect(), ref dropClips2))
							{
                                if ( dropClips2.Length > 0 )
                                {
                                    int firstIndex = subItemCount;
                                    currentSubitemIndex = firstIndex;
                                    foreach ( AudioClip audioClip in dropClips2 )
                                    {
                                        ArrayHelper.AddArrayElement( ref curItem.subItems ).Clip = audioClip;
                                        currentSubitemIndex++;
                                    }
	...
	bool DropAudioClipsInArea(Rect rect, ref UnityEngine.AudioClip[] actualDragAssets)
	{
		...
		switch(evt.type)
		{
			...
			// фильтруем пересаскиваемые ассеты
			actualDragAssets = DragAndDrop.objectReferences.Where(s => s is AudioClip).Select(s => (AudioClip)s).ToArray();
			if (actualDragAssets.Length == 0)
				return false;
